 ...
 */
class FileslistField extends BaseField
{
    /**
    ...
    public function label()
    {
        ...
        if (version_compare(Kirby::version(), '2.2', '>=')) {
            ...
            $action->attr('href', purl($this->page(), 'upload'));
				...
				/* Modify button */
        $modify = new Brick('a');
        $modify->addClass('file-modify-button label-option');
        $modify->html('<i class="icon icon-left fa fa-sort"></i>' . "Classer");
				$modify->attr('href', $this->page()->url('files'));
				$modify->attr('style', "right: 100px;");
